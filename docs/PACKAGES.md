...
| `internal/world` | ✅ | Chunks, blocks, block-state properties, tick state, crack tracking. **Phase 4a:** unified per-world tick loop (`internal/world/tick.go`) owns the 7-phase cadence (player inputs → scheduled ticks → random ticks → mob AI → drop physics → outbound stage → autosave) at 20 Hz; the legacy `StartTimeLoop`/`StartMobAI`/`StartDropPhysics`/`StartAutosave` are now thin wrappers. **Phase 4b:** lighting engine (`internal/world/light.go`) — BFS sky + block light propagation from sources, per-state opacity/emission lookup table, chunk-level light compute on load, deferred recompute queue on block change (processed in tick phase 3b). Mob spawn director uses real light levels (≤7 threshold for hostiles). Java chunk export reads real light nibbles; Bedrock defers to client-side lighting. | R3,R4,R3.7 |
