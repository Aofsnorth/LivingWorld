...
// ReadFrom reads the packed longs with no length prefix. The destination slice
// b.data must already be sized to the expected long count (PaletteContainer
// pre-sizes it from bits-per-entry and the container length). See WriteTo for
// the protocol 1.21.5+ format note.
...
// WriteTo writes the packed longs with no length prefix.
//
// Protocol 1.21.5+ (774+): the paletted container's "Data Array Length" VarInt
// was removed — the client computes the long count from bits-per-entry and the
// container size. A bits==0 (single-value) container therefore writes nothing.
func (b *BitStorage) WriteTo(w io.Writer) (n int64, err error) {
	if b == nil {
		return 0, nil
	}
