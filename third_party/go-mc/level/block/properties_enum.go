// Code generated by tools/gen_blocks.go from 26.1/block_properties.json; DO NOT EDIT.
...
var strPose = [...]string{"standing", "sitting", "running", "star"}

func (p Pose) String() string {
	if int(p) < len(strPose) {
		return strPose[p]
	}
	return "invalid Pose"
}

func (p Pose) MarshalText() (text []byte, err error) {
	if int(p) < len(strPose) {
		return []byte(strPose[p]), nil
	}
	return nil, errors.New("invalid Pose: " + strconv.Itoa(int(p)))
}

func (p *Pose) UnmarshalText(text []byte) error {
	switch str := string(text); str {
	case "standing":
		*p = PoseStanding
	case "sitting":
	...
	case "star":
		*p = PoseStar
	default:
		return errors.New("unknown Pose: " + str)
	}
	return nil
}
