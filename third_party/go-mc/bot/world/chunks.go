...
func (w *World) handleLevelChunkWithLightPacket(packet pk.Packet) error {
	...
	// Data in RawMessage misses the compound type and name length, so we can't unmarshal it directly.
	// As a quick fix for the bot, just fallback to 384 or 256.
	chunk := level.EmptyChunk(384 / 16)
